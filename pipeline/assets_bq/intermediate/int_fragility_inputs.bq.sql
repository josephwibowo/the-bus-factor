...
components AS (
    SELECT
        ...
        CASE
            ...
            WHEN DATE_DIFF(j.snapshot_week, j.latest_release_date, DAY)
                <= {{ var.threshold_release_recency_floor_days }} THEN 0
            WHEN DATE_DIFF(j.snapshot_week, j.latest_release_date, DAY)
                >= {{ var.threshold_release_recency_cap_days }} THEN 100
            ELSE (
                DATE_DIFF(j.snapshot_week, j.latest_release_date, DAY)
        ...
        CASE
            ...
            WHEN DATE_DIFF(j.snapshot_week, j.last_commit_date, DAY)
                <= {{ var.threshold_commit_recency_floor_days }} THEN 0
            WHEN DATE_DIFF(j.snapshot_week, j.last_commit_date, DAY)
                >= {{ var.threshold_commit_recency_cap_days }} THEN 100
            ELSE (
                DATE_DIFF(j.snapshot_week, j.last_commit_date, DAY)
