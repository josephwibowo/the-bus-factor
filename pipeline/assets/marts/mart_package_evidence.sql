...
SELECT ecosystem, package_name, 'all_time_contribution_concentration',
    ...
    CASE
        ...
        ELSE 'Top contributor authored '
             || CAST(ROUND(top_contributor_share_all_time * 100.0, 1) AS VARCHAR)
             || '% of all-time commits (distinct contributors: '
             || CAST(COALESCE(contributors_all_time, 0) AS VARCHAR) || ').'
    END,
    snapshot_week
FROM base
UNION ALL
SELECT ecosystem, package_name, 'recent_commit_concentration_365d',
    ...
    CASE
        ...
        ELSE 'Top contributor authored '
             ...
             || CAST(COALESCE(unique_contributors_last_365d, 0) AS VARCHAR) || ').'
    END,
    snapshot_week
FROM base
UNION ALL
