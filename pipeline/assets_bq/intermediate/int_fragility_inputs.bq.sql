...
components AS (
    SELECT
        ...
        j.package_name,
        j.snapshot_week,
        CAST(DATE_DIFF(j.snapshot_week, j.latest_release_date, DAY) AS INTEGER)
            AS days_since_release,
        CAST(DATE_DIFF(j.snapshot_week, j.last_commit_date, DAY) AS INTEGER)
            AS days_since_commit,

        -- release_recency: floor 90, cap 540
        CASE
            WHEN j.latest_release_date IS NULL THEN 0
            WHEN DATE_DIFF(j.snapshot_week, j.latest_release_date, DAY) <= 90 THEN 0
            WHEN DATE_DIFF(j.snapshot_week, j.latest_release_date, DAY) >= 540 THEN 100
            ELSE (DATE_DIFF(j.snapshot_week, j.latest_release_date, DAY) - 90)
                 / (540.0 - 90.0) * 100.0
        ...
        CASE
            ...
            WHEN DATE_DIFF(j.snapshot_week, j.last_commit_date, DAY) <= 30 THEN 0
            WHEN DATE_DIFF(j.snapshot_week, j.last_commit_date, DAY) >= 365 THEN 100
            ELSE (DATE_DIFF(j.snapshot_week, j.last_commit_date, DAY) - 30)
                 / (365.0 - 30.0) * 100.0
